...
from backend.services.async_wrappers import (
...
class OnboardingPayloadStep3(BaseModel):
    ...
    - story_core: 故事内核 (100+ 章体量的故事骨架: 主角是谁 + 场景/环境 + 做什么 + 遇到什么意外 + 留悬念)
    - characters: 主要角色 (主角/对手/盟友, 每行 '名字 - 身份/角色 - 特点/目的')
...
async def onboarding(project_id: str, req: OnboardingRequest):
    ...
    await conv_repo.add_message(
        ...
        tool_results={"name": "onboarding_step", "args": {"step": req.step, "payload": req.payload},
...
async def submit_intervention(project_id: str, req: InterventionRequest):
    ...
    await conv_repo.add_message(
        conversation_id=conv.id,
        role=MessageRole.TOOL,
        tool_results={"name": "intervene",
                      "args": {"intervention": req.intervention, "actor_feedback": req.actor_feedback,
...
async def rollback_project(
    ...
    from backend.agent.tools.schemas import RollbackBaseInput
    ...
    await conv_repo.add_message(
        conversation_id=conv.id,
        role=MessageRole.TOOL,
        tool_results={"name": "rollback_to_node",
...
async def generate_image(project_id: str, req: ImageRequest):
    ...
    await conv_repo.add_message(
        conversation_id=conv.id,
        role=MessageRole.TOOL,
        tool_results={"name": "generate_image",
