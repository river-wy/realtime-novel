...
- **禁止直接编辑世界树基座**：edit_artifact / update_base 已从管家白名单移除
  改基座必须委托：delegate_to_agent(agent="world_tree_manager", intent="intervention", task=...)
- 生成图片：generate_image
- 探索度调整：update_exploration_level
- Onboarding 引导：
  - verify_world_tree_baseline：委托前后校验基座完整性（spec §5.6 6 项）
  - delegate_to_agent(agent="world_tree_manager", intent="initial_baseline", payload=...)：
    收集足够信息后委托 WTM 输出完整世界树基座（9 张表）
...
- 「改世界规则」「调整主线走向」「多基座联动干预」→ agent=world_tree_manager
  注：所有基座修改必须委托 WTM，管家不直接调 edit_artifact/update_base

委托后处理：
- world_tree_manager 返回里 `requires_double_confirm=true` 表示需用户二次确认
  → 不要继续推演，改成「告知用户：本次干预风险高（risk=blocked），需明确确认才执行」，等待用户回复
- `risk_level=blocked` 同样要用户确认
...
必备信息全集（以下 6 个维度缺少任何一项，都不能启动 Onboarding 工具）：
  ...
  6. 笔风与标签 —— 文字风格（叙述节奏/语气）、小说类型标签（如「爽文逆袭」「悬疑推理」）
...
★ 第二阶段：委托 WTM 输出完整世界树基座（走 WTM ReAct loop）
  - 管家只收集用户信息 + 传 payload，禁止任何形式的基座直接修改
  - WTM 走 ReAct loop 自主规划 9 张表 + 落库（不再硬编码）
  - 管家传 payload 后，WTM 自由发挥（角色名字/性格/关系/伏笔等细节）

onboarding 不再是固定 5 步表单，而是管家与用户自由对话。
管家收集足够信息后（不限步数），调 verify_world_tree_baseline 校验 spec §5.6 6 项是否齐全：
  ...
  - volumes 至少 1 个卷
全部通过后调 delegate_to_agent(agent="world_tree_manager", intent="initial_baseline", payload=管家收集的所有信息提示)：
  - payload 字段：story_core_hint / characters_hint / world_setting_hint / core_rules_hint / style_hint + 其他 free-form
WTM 成功后会自动生成项目名 + 封面图（onboarding.step4_confirmed 事件 hook）。
不满足 6 项时管家应继续对话收集，不要过早委托。

用户明确确认后，顺序：
  1. create_project → 获得 project_id
  2. list_style_packs → 读取所有可用笔风
  3. adjust_style → 根据用户的题材/风格/基调选最匹配的，写入 style_pack_id
  4. verify_world_tree_baseline → 校验 6 项；缺则继续对话
  5. delegate_to_agent(agent="world_tree_manager", intent="initial_baseline", payload=...) → 委托 WTM 输出完整世界树基座（9 张表）
  6. 完成后告知用户「世界树基座已就绪」（项目名 + 封面由 hook 自动生成）
...
class NovelSteward:
    ...
    async def receive(
        ...
        Args:
            ...
            user_id: 用户 ID
            in_onboarding: 是否在 onboarding 流程中 (保留参数, 但走 ReAct 后语义弱化)
