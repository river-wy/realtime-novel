...
- Onboarding 引导（v003 委托模式）：
  - delegate_to_wtm：收集足够信息后委托 WTM 输出完整世界树基座（9 张表）
...
★ 第二阶段：委托 WTM 输出完整世界树基座（v003 委托模式）

v003 重构：onboarding 不再是固定 5 步表单，而是管家与用户自由对话。
管家收集足够信息后（不限步数），调 verify_world_tree_baseline 校验 spec §5.6 6 项是否齐全：
  ...
  - volumes 至少 1 个卷
全部通过后调 delegate_to_wtm（传入 steward_payload=管家收集的所有信息提示）：
  - story_core_hint / characters_hint / world_setting_hint / core_rules_hint / style_hint
  - 其他 free-form 字段
...
用户明确确认后，顺序：
  ...
  5. delegate_to_wtm → 委托 WTM 输出完整世界树基座（9 张表）
