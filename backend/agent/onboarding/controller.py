...
- OnboardingController.consult(step=3/4) 已在 v0.8 被 WorldTreeManager.run_initial_baseline_react() 替代。

所有调用方已迁移：
- onboarding_tools.py step 1/2 → 直接读 onboarding_state.payload 字段返回
- onboarding_tools.py step 3   → delegate WorldTreeManager.run_initial_baseline_react()
- onboarding_tools.py step 4   → 读 DB artifacts

v0.9.4 修正：initialize_world_tree() 老版入口已删（v0.6.2 命名 run_initial_baseline_react）。
