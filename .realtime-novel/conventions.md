...
| v1.1 | 2026-06-15 | M-γ 后：强约束 #14（生成产物随项目保存）+ #15（调试临时文档入 .tmp/）= 15 条强约束 |
...
| 15 | **调试临时文档放 `<workspace>/.tmp/`**（工程根隐藏目录），**不入仓**。调试 LLM 输出 / 生成中间产物 / 任何不打算提交的临时文件都进这里。`.gitignore` 已覆盖 `.tmp/`（**必须用 `.tmp/`，不能用 `tmp/`，避免与 `*.tmp` 后缀冲突**）。**禁止**用 `~/.openclaw/tmp/` 等系统级临时目录放本工程的临时文档（避免散落难找）|
...
| 2026-06-15 | v1.1: #14 + #15 两条强约束（欧尼酱 19:27 拍板 .tmp/ 目录）| 蕾姆酱 🩵 |
