...
现在: 统一 logger = realtime_novel.{module}, 输出到 stderr (uvicorn 捕获) + tmp/logs/backend.log.

用法:
    from realtime_novel.utils.logger import get_logger
    log = get_logger(__name__)
    log.info("LLM 调用开始: step=%s, msg_count=%d", step, len(messages))
    log.error("LLM 输出解析失败: %s", e, exc_info=True)

设计:
    - 命名空间: realtime_novel.* (沿用 Python logging 惯例)
