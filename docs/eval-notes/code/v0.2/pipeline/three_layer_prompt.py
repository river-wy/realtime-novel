...
def build_full_prompt(
    ...
    char_lines = []
    for c in character_card.get("characters", []):
        char_lines.append(
    ...
    # === 当前 beat（防止 LLM 跑偏主线）===
    current_beat = None
    for b in main_plot.get("beats", []):
        # 用 chapter_range 之外的信息判断当前
        if b["status"] == "active":
            current_beat = b
            break
    if not current_beat and main_plot.get("beats"):
        current_beat = main_plot["beats"][0]
    beat_text = ""
    if current_beat:
        beat_text = f"""## 当前主线节拍
- ID: {current_beat['id']}
...
=== 输出要求 ===
请生成下一章正文，~3000 字散文式。
【严格遵守】人物名字使用上述「人物设定」中给出的名字，**不要改名**。
"""
    return full
