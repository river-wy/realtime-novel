...
|---|---|---|---|---|---|---|
| **项目管理** | `load_project` | `project_tools.py:50` | `project_id` | `ProjectDetail`（id / name / world_tree / chapters） | 管家/文笔家/架构师/Validator 几乎每个任务开头 | ❌ |
| **项目管理** | `list_projects` | `project_tools.py:144` | 无 | `ListProjectsOutput` | 管家（"我有什么项目"用，v0.9.1 新增） | ❌ |
| **项目管理** | `create_project` | `project_tools.py:80` | `name` / `exploration_level` (conservative/standard/wild) / `initial_prompt` / `style_pack_id` | `ProjectDetail` | 管家 Onboarding 阶段 | ❌ |
