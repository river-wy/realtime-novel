...
| 4 | `api.createProject(name, palette, prompt)` `projects.ts:60` | `/api/projects` (`project_routes.py:103`) | POST | ✅ 对齐 | 后端返回 `CreateProjectResponse{ id, name, created_at, onboarding_required }`；前端未取 `onboarding_required` |
...
> 注 1：`palette` 在 list 接口下后端硬编码为 `""`（`services/project_manager.py:148`），前端展示时显示空字符串。无崩溃风险但 UX 上「主题色」永远空白。

#### `ProjectDetail` interface（L25-37） vs 后端 `ProjectDetailResponse` (`project_routes.py:58-70`)
| 前端字段 | 后端实际返回 | 状态 |
|---------|------------|------|
| `id/name/palette/exploration_level/seven_artifacts/world_tree/chapters/cover_image_url` | ✓ | ✅ |
...
#### P2-4：`list` 端点 `palette` 始终为空
- 位置：`services/project_manager.py:148` 硬编码 `""`
- 风险：World/WorldList 页面 `palette-badge` 永远空白
- 建议：从 `onboarding_payload.palette` 取值（`project_manager.py:119` 已有该逻辑），或在 list 返回中合并
